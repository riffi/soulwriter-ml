...
def pattern_to_regex(pattern_str):
  ...
  for token in tokens:
    if token.startswith('(') and token.endswith(')'):
      ...
      regex_parts.append(r'(\w+)')  # Корректная raw-строка
    else:
      escaped = re.escape(token.lower())
      regex_parts.append(escaped)

  # Собираем выражение с правильными escape-последовательностями
  return r'\b' + r'\s+'.join(regex_parts) + r'\b'
...
def load_patterns():
  ...
  for clishe in clishes:
    patterns.append(re.compile(pattern_to_regex(clishe), flags=re.UNICODE | re.IGNORECASE))
  return patterns

def find_matches(text, regex_patterns):
  """Ищет совпадения в тексте."""
  matches = []
